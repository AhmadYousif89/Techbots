...
export async function PopularItemsSlide() {
  ...
  const watches = (await getProductsByCategory('watches')).slice(0, 2);
  const mobiles = (await getProductsByCategory('mobiles')).slice(0, 2);
  const powerbanks = (await getProductsByCategory('powerbanks')).slice(5, 7);
  const products = [...laptops, ...mobiles, ...watches, ...powerbanks];

  return (
    <section className='bg-secondary py-10 px-6'>
      <h2 className='text-2xl font-bold mb-12'>Popular Items</h2>
      <Carousel
